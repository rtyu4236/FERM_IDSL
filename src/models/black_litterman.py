...
class BlackLittermanPortfolio:
    ...
    def get_black_litterman_portfolio(self, analysis_date, P, Q, Omega, pre_calculated_inputs=None, max_weight=0.25, previous_weights=None):
        ...
        # mu_bl (posterior returns)에서 ETF 비용(expense ratio)을 차감합니다.
        # 비용은 연간 단위이므로 12로 나누어 월간 비용으로 변환합니다.
        expenses_arr = np.array([self.expense_ratios.get(permno, {}).get('expense_ratio', 0.0) for permno in current_permnos])
        
        # logger.info(f"Expenses array for {len(current_permnos)} assets: {expenses_arr}")

        # mu_bl과 expenses_arr가 연산 가능한 형태인지 확인합니다.
        # logger.info(f"[BlackLittermanPortfolio.get_black_litterman_portfolio] Before expense deduction: mu_bl shape={getattr(mu_bl, 'shape', 'N/A')}, expenses_arr shape={expenses_arr.shape}")

        mu_bl_arr = np.asarray(mu_bl, dtype=float)
        
        # 순수 기대수익률(mu_bl_net)을 계산합니다。
