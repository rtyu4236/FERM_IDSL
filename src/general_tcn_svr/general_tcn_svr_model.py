...
def train_general_tcn_svr(daily_df, monthly_df, vix_df, ff_df, all_permnos, train_start_year, train_end_year, model_params, use_etf_ranking, top_n, run_rolling_tune, liquid_universe_dict=None):
    ...
    analysis_date = backtest_dates[0]
    liquidity_log_records = []  # [{'as_of': date, 'before_count': int, 'after_count': int}]
    ranking_selected_per_month = {} if use_etf_ranking else None
    
    # TCN_SVR 하이퍼파라미터 튜닝
    tcn_svr_tuner.run_tuning(ml_features_df, n_trials=config.MODEL_PARAMS['tcn_svr_params']['tune_trials_per_month'])
    current_model_params = config.get_model_params()
    tcn_svr_model_params = current_model_params['tcn_svr_params']
    
    model = TCN_SVR_Model(
        input_size=15, # len(all_features),
        output_size=7,# len(indicator_features),
        num_channels=tcn_svr_model_params['num_channels'],
        kernel_size=tcn_svr_model_params['kernel_size'],
        dropout=tcn_svr_model_params['dropout'],
        lookback_window=tcn_svr_model_params['lookback_window'],
        svr_C=tcn_svr_model_params.get('svr_C', 1.0),
        svr_gamma=tcn_svr_model_params.get('svr_gamma', 'scale'),
        lr=tcn_svr_model_params.get('lr', 0.001) # Pass the tuned learning rate
    )
    ...
    for idx, analysis_date in enumerate(backtest_dates[:-1]):
        ...
        # 2) 유동성 필터를 통과한 전체 중 top_N * 3/5개, 나머지 중 랜덤하게 top_N * 2/5개 선정
        if use_etf_ranking and ranker:
            ...
            random_permnos = random.sample(list(set(candidate_permnos) - set(ranked_permnos)), 20)
            universe_for_month = ranked_permnos + random_permnos
        ...
        current_permnos, _ = get_current_universe(all_returns_df=monthly_df_filtered, analysis_date=analysis_date, lookback_months=tcn_svr_model_params.get('lookback_window'))
        ...
        for i, permno in enumerate(current_permnos):
            ...
            X_train_tensor, y_indicators_tensor, y_returns_seq, _ = generate_permno_tensors(
                permno, full_feature_df, indicator_features, all_features, train_window_rows, tcn_svr_model_params
            )
